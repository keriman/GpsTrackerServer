...
class recorridos
{
  ...
  $sql5 = "SHOW FULL TABLES FROM ssptracker";
  $oktable = mysqli_query($con,$sql5);

 while(($datos = mysqli_fetch_assoc($oktable))>0){

   if(substr($datos['Tables_in_ssptracker'],0,3)=="are"){

    $ID_A[$r] = substr($datos["Tables_in_ssptracker"],(-1)*strlen($datos["Tables_in_ssptracker"])-9);

    $sql_aux1[$r] = "SELECT latitud,longitud,ID_A,speed,altitud,direccion,disUlt,error,TimeGPS
               FROM ".$datos['Tables_in_ssptracker']."
               WHERE TimeGPS = (SELECT MAX(TimeGPS) FROM ".$datos['Tables_in_ssptracker'].")";
      $r++;
   }
 }//while
